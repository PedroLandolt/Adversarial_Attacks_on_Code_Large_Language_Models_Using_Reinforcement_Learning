...
def _project_root() -> Path:
    """Return the project root (parent of V3/)."""
...
def _load_synthesized_wbo(path: Path) -> list:
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            f"Build it: python V3/utils/operator_mutation.py"
        )
    samples = []
    with open(path, encoding="utf-8") as f:
        for line in f:
...
def _load_cubert_wbo(limit: int = _CUBERT_WBO_LIMIT) -> list:
    ...
    The local file must be built first with:
        python V3/utils/download_cubert_wbo.py
    ...
    if normalized == "adversarial_code_buggy":
        ...
        if not samples:
            raise FileNotFoundError(
                ...
                "Build them first with: python V3/utils/pregenerate_buggy_code.py"
            )
