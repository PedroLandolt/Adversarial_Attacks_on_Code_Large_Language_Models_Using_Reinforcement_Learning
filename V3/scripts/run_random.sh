#!/usr/bin/env bash
# run_random.sh — Random choice baseline
#
# Usage:
#   bash V3/scripts/run_random.sh                          # both benchmarks
#   bash V3/scripts/run_random.sh adversarial_code_buggy
#   bash V3/scripts/run_random.sh cubert_wbo
#   MAX_ITERATIONS=3 bash V3/scripts/run_random.sh
set -euo pipefail
...
LOG="logs/experiment/random_$(date +%Y%m%d_%H%M%S).log"

run() {
    local BM="$1"
    echo "[$(date '+%H:%M:%S')] random_choice — $BM (max_iter=$MAX_ITERATIONS)" | tee -a "$LOG"
