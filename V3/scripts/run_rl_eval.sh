...
#
# Run AFTER run_rl_train.sh. Auto-detects latest epoch checkpoint unless WEIGHTS_PATH is set.
#
# Usage:
#   bash V3/scripts/run_rl_eval.sh                          # both benchmarks, UCB1
#   bash V3/scripts/run_rl_eval.sh adversarial_code_buggy
#   bash V3/scripts/run_rl_eval.sh cubert_wbo
#   BANDIT_ALGORITHM=thompson WEIGHTS_PATH=weights/acb_thompson_llama31_qwen.json bash V3/scripts/run_rl_eval.sh adversarial_code_buggy
#
# Supported algorithms: ucb1, thompson, klucb, exp3
# WEIGHTS_PATH: if set, skips auto-detection and uses the specified file
set -euo pipefail
...
eval_benchmark() {
    ...
    if [[ -n "$WEIGHTS_PATH_OVERRIDE" ]]; then
        ...
        WEIGHTS="$("$PYTHON" -c "
import glob, os
alg = os.environ.get('BANDIT_ALGORITHM', 'ucb1')
bm = '${BM}'
checkpoints = sorted(glob.glob('weights/' + bm + '_' + alg + '_epoch_*.json'))
base = 'weights/' + bm + '_' + alg + '.json'
chosen = checkpoints[-1] if checkpoints else base
print(os.path.abspath(chosen))
" BANDIT_ALGORITHM="$BANDIT_ALGORITHM")"
    fi

    if [[ ! -f "$WEIGHTS" ]]; then
        echo "[$(date '+%H:%M:%S')] ERROR: no weights found for $BM / $BANDIT_ALGORITHM — run run_rl_train.sh first" | tee -a "$LOG"
        return 1
    fi
    ...
    for SPLIT in validation test; do
        ...
        PYTHONPATH=V3 "$INSPECT" eval V3/adversarial_attack.py@adversarial_code_llm \
            ...
            -T split_definition="${BM}:full:${SPLIT}" \
            2>&1 | tee -a "$LOG"
    done
