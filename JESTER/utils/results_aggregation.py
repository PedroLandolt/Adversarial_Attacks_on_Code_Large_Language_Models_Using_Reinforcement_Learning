...
def aggregate_persisted_runs(
    *,
    ...
    for run in loaded_runs:
        ...
        # Compute per-iteration wins from attempt-level data (each stored_results
        # folder is a single-sample run, so these are 0.0 or 1.0 per run).
        attempts = run.get("attempts") or []
        def _iter(a: dict) -> int:
            try:
                return int(a.get("iteration", 99))
            except (TypeError, ValueError):
                return 99

        baseline_win = float(any(
            a.get("attack_success") and _iter(a) == 0 for a in attempts
        ))
        asr_1shot_win = float(any(
            a.get("attack_success") and _iter(a) <= 1 for a in attempts
        ))
        asr_2shot_win = float(any(
            a.get("attack_success") and _iter(a) <= 2 for a in attempts
        ))
        asr_3shot_win = float(any(
            a.get("attack_success") and _iter(a) <= 3 for a in attempts
        ))
        asr_4shot_win = float(any(
            a.get("attack_success") and _iter(a) <= 4 for a in attempts
        ))
        asr_5shot_win = float(any(
            a.get("attack_success") and _iter(a) <= 5 for a in attempts
        ))
        asr_6shot_win = float(any(
            a.get("attack_success") and _iter(a) <= 6 for a in attempts
        ))
        asr_tactic_win = float(any(
            a.get("attack_success") and _iter(a) == 1 for a in attempts
        ))
        ...
        run_rows.append(
            {
                ...
                # Per-iteration ASR fields (primary metric = asr_1shot_win)
                "baseline_win": baseline_win,
                "asr_1shot_win": asr_1shot_win,
                "asr_2shot_win": asr_2shot_win,
                "asr_3shot_win": asr_3shot_win,
                "asr_4shot_win": asr_4shot_win,
                "asr_5shot_win": asr_5shot_win,
                "asr_6shot_win": asr_6shot_win,
                "asr_tactic_win": asr_tactic_win,
        ...
        grouped_summary.append(
            {
                ...
                "mean_asr_1shot_win": _average(acc["asr_1shot_win"]),
                "mean_asr_2shot_win": _average(acc["asr_2shot_win"]),
                "mean_asr_3shot_win": _average(acc["asr_3shot_win"]),
                "mean_asr_4shot_win": _average(acc["asr_4shot_win"]),
                "mean_asr_5shot_win": _average(acc["asr_5shot_win"]),
                "mean_asr_6shot_win": _average(acc["asr_6shot_win"]),
