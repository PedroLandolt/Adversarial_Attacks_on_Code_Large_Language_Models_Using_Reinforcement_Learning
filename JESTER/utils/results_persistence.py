...
def load_persisted_runs(results_dir: str) -> list[dict]:
    ...
    # Recursive scan: find every run_summary.json anywhere under results_dir
    # and treat its parent as the run directory. This supports both the flat
    # structure used by results/ and the nested structure of stored_results/.
    candidate_dirs = sorted(p.parent for p in results_root.rglob("run_summary.json"))
