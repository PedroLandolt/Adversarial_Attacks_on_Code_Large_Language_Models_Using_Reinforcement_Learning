...
Workflow:
  ...
  3. Run this script: python JESTER/utils/validate_manual_buggy.py
...
Usage:
    python JESTER/utils/validate_manual_buggy.py               # process all filled-in records
    python JESTER/utils/validate_manual_buggy.py --dry-run     # show results, no writes
    python JESTER/utils/validate_manual_buggy.py --judge-model qwen2.5-coder:7b
    python JESTER/utils/validate_manual_buggy.py --ollama-url http://localhost:11434
...
def main() -> None:
    ...
    print("Next steps:")
    print("  python JESTER/utils/upload_to_hub.py   # rebuild combined JSONL + upload to HuggingFace")
