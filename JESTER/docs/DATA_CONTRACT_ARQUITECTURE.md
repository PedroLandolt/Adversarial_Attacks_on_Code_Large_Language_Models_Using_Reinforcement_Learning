...
2. **Gitea** as a separate **reviewer-approval** environment built around the protected `JESTER/gitea/` contract.
...
`JESTER/gitea/` is protected externally provided code. Its base `tools.py` and `schemas.py` define the environment contract and must not be rewritten. Thesis work must integrate around that contract.
