"""
Main evaluation task for the thesis project.

Task
----
adversarial_code_llm:
    Iterative adversarial attack loop over a buggy-code corpus.
...
Notes
-----
- The selector, attack generation, and evaluation logic remain conceptually separated.
"""
...
def adversarial_code_llm(
    ...
    # Enforce 70/15/15 split by slicing the dataset before any evaluation.
    # experiment_split="full" skips filtering (smoke tests / one-off runs).
    if experiment_split in {"train", "validation", "test"}:
        all_samples = [s for s in base_task.dataset]
        n = len(all_samples)
        train_end = int(n * 0.70)
        val_end = train_end + int(n * 0.15)
        if experiment_split == "train":
            subset = all_samples[:train_end]
        elif experiment_split == "validation":
            subset = all_samples[train_end:val_end]
        else:  # test
            subset = all_samples[val_end:]
        base_task = Task(dataset=MemoryDataset(subset), scorer=base_task.scorer)
